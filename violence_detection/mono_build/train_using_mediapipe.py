...
class KeypointCollector:
    def __init__(self, sequence_length=10):
        ...
        self.current_sequences = [deque(maxlen=sequence_length) for _ in range(6)]  # Up to 6 people
    ...
    def process_frame(self, frame):
        ...
        if results.pose_landmarks:
            # Extract 33 landmarks for each person
            for i, pose_landmarks in enumerate([results.pose_landmarks]):  # Assuming one person per frame
                landmarks = [(lm.x, lm.y, lm.visibility) for lm in pose_landmarks.landmark]
...
def preprocess_sequences(sequence_data):
    X = np.array([seq.reshape(-1, 33 * 3) for seq, _ in sequence_data])  # 33 * 3 = 99 features
...
def build_lstm_model(seq_length=10, feature_size=99):  # Updated for 33 landmarks
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(seq_length, feature_size), name='lstm_input'),  # Explicit input layer
...
def convert_to_tflite(keras_model, output_path="violence_mediapipe.tflite"):
    ...
    input_shape = (1, 10, 99)  # Fixed batch size for TFLite (33 * 3 = 99)
    input_tensor_name = 'lstm_input'  # Match the name from Input layer
    converter.input_shapes = {input_tensor_name: input_shape}
    
    # Convert the model
    try:
        tflite_model = converter.convert()
        print(f"TFLite model successfully converted.")
...
def main():
    ...
    sequence_data = video_violent_sequences + video_non_violent_sequences
    ...
    lstm_model = build_lstm_model(seq_length=10, feature_size=99)  # Updated for 33 landmarks
    ...
    # Save the model
    lstm_model.save("violence_mediapipe.h5")
    print("Keras model saved as 'violence_mediapipe.h5'")

    # Convert to TFLite
    print("Converting model to TFLite format...")
    convert_to_tflite(lstm_model, "violence_mediapipe.tflite")
