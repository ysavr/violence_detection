...
class MoveNetViolenceDetector:
    def __init__(self, sequence_length=10):
        ...
        self.violence_positions = []  # List to store (frame_number, timestamp) pairs
        self.current_sequences = [deque(maxlen=sequence_length) for _ in range(6)]
        self.start_time = None  # Start time for FPS calculation
        self.real_fps = 0.0     # Store real-time FPS

        # Load model using .h5 model
        try:
            self.lstm_model = tf.keras.models.load_model("violence_lstm_model.h5")
            print("LSTM model loaded successfully")
        except Exception as e:
            print(f"Error loading LSTM model: {e}. No violence detection possible.")
        ...
        for i, person in enumerate(keypoints_with_scores):
            if np.max(person[:, 2]) > 0.1:
                ...
                if len(self.current_sequences[i]) == self.sequence_length:
                    seq = np.array(self.current_sequences[i]).reshape(1, self.sequence_length, 17 * 3)
                    prediction = self.lstm_model.predict(seq, verbose=0)[0][0]
                    if prediction > 0.5:
                        return True
        return False
    ...
    def process_frame(self, frame, frame_number, fps):
        ...
        # Display real-time FPS on the frame
    ...
    def detect_in_video(self, video_path=None):
        ...
        self.start_time = time.time()  # Start timing for real-time FPS
        frame_number = 0
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
        ...
        if self.lstm_model:
            for i, person in enumerate(keypoints_with_scores):
                if np.max(person[:, 2]) > 0.1:
                    ...
                    seq = np.array(sequence).reshape(1, self.sequence_length, 17 * 3)
                    prediction = self.lstm_model.predict(seq, verbose=0)[0][0]
                    if prediction > 0.5:
                        self.violence_frames = 1
                        cv2.putText(frame, "VIOLENCE DETECTED", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def main():
    ...
    detector = MoveNetViolenceDetector(sequence_length=10)
    
    # Test with video
    video_path = "../../data/video/testing/testing.mp4"
